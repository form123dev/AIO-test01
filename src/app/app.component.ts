...
export class AppComponent implements OnInit {
  ...
  */
  ...
  ngOnInit() {
    ...
    이 루트 구성 요소가 절대로 죽지 않으므로 가입을 취소 할 필요가 없습니다.
    */
    ...
    this.locationService.currentPath.subscribe(path => {
      ...
      // 보관 모드이고 문서 페이지를 방문하지 않는 경우 docs로 리디렉션
      // (즉, 마케팅 페이지에 도착했습니다)
      ...
      if (path === this.currentPath) {
        ...
        // 같은 페이지에있는 경우에만 스크롤합니다 (해시 값 변경 가능성이 높음).
        this.autoScroll();
      } else {
        ...
        // 짧은 시간 내에 문서가 렌더링되지 않으면 진행률 표시 줄 시작
    ...
    // 현재 버전의 버전 선택기 목록과 탐색 맵의 버전을 계산합니다.
    combineLatest(
  ...
  // 아래에서  Click
  // Click  발생하면 여기에서 Clink 된 것이 무엇인지 분석하여 처리하나다.
